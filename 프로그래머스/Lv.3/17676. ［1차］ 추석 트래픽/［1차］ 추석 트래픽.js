...
function solution(lines) {
    ...
    for(const line of lines) {
        const [date, completeTime, duration] = line.split(' ');
        const completeMillis = transTimeToMilliSeconds(completeTime)
        const endTime = completeMillis + 999;
        const startTime = completeMillis - duration.substr(0, duration.length-1)*1000 + 1;
